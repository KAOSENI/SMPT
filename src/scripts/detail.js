...
//   - "#detail-dynamic" (métricas, fases, equipos, eventos): esto sí se
//     reescribe en cada tick, porque son textos/colores que cambian seguido
//     y no producen parpadeo visible al hacerlo.
...
function dynamicHtml(tx, s, id) {
  ...
  if (!anyInstalled) {
    ...
    equipHtml = `<div style="margin-bottom:12px;">${rows}</div>`;
  }
  ...
  const eventsHtml = txEvents.length > 0 ? txEvents.map(e => `
...
export function openDetail(id) {
  ...
  // no toca las gráficas ni el resto del cascarón.
  const dynamicEl = document.getElementById('detail-dynamic');
  dynamicEl.innerHTML = dynamicHtml(tx, s, id);
  dynamicEl.querySelectorAll('[data-equip-on]').forEach(el => {
